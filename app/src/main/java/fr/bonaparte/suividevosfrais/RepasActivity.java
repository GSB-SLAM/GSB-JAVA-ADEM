 ...
 * RepasActivity.java
 * 
 * Classe RepasActivity qui hérite de la classe Activity.
 * Elle est associée au layout activity_repas.xml.
 * Permet d'afficher et de rentrer les frais 
 * repas du mois en cours.
 *
 */
public class RepasActivity extends Activity {
	...
	private Integer qte;// Variable pour récupérer la quantité de repas (unitaire)
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//Désactive la saisie dans le input Text repas
		((EditText)findViewById(R.id.txtRepas)).setInputType(InputType.TYPE_NULL);
	}
	...
	private void valoriseProprietes() {
		...
		//On récupère le frais forfaitaire repas à la date sélectionnée
		fraisF = fraisFBdd.getFraisFDate("repas", uneDate);
	...
	private void imgReturn_clic() {
		findViewById(R.id.imgRepas).setOnClickListener(new ImageView.OnClickListener() {
    ...
    private void cmdMoins_clic() {
    	((Button)findViewById(R.id.cmdRepasMoins)).setOnClickListener(new Button.OnClickListener() {
    		public void onClick(View v) {
   				qte = Math.max(0, qte-1);// Enlève 1 repas
	...
	private boolean enregNewQte() {
		...
		sinon on rajoute un 0 devant*/
            ...
            fraisF = new FraisF(uneDate, "repas", qte);
